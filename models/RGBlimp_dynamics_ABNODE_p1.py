...
class ABNODE_p1(ODEF):
    ...
    def __init__(self,RGBlimp_params,device):
        ...
        self.I = torch.diag(torch.tensor([self.Ix,self.Iy,self.Iz],dtype=torch.double,requires_grad=False)).to(self.device)

        self.K1 = nn.Parameter(torch.tensor([RGBlimp_params['K1']],dtype=torch.double,requires_grad=True).to(self.device))
        self.K2 = nn.Parameter(torch.tensor([RGBlimp_params['K2']],dtype=torch.double,requires_grad=True).to(self.device))
        self.K3 = nn.Parameter(torch.tensor([RGBlimp_params['K3']],dtype=torch.double,requires_grad=True).to(self.device))

        self.Cd0 = nn.Parameter(torch.tensor([RGBlimp_params['Cd0']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cda = nn.Parameter(torch.tensor([RGBlimp_params['Cda']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cdb = nn.Parameter(torch.tensor([RGBlimp_params['Cdb']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cl0 = nn.Parameter(torch.tensor([RGBlimp_params['Cl0']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cla = nn.Parameter(torch.tensor([RGBlimp_params['Cla']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Clb = nn.Parameter(torch.tensor([RGBlimp_params['Clb']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmy0 = nn.Parameter(torch.tensor([RGBlimp_params['Cmy0']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmya = nn.Parameter(torch.tensor([RGBlimp_params['Cmya']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmyb = nn.Parameter(torch.tensor([RGBlimp_params['Cmyb']], dtype=torch.double,requires_grad=True).to(self.device))

        self.Cs0 = nn.Parameter(torch.tensor([RGBlimp_params['Cs0']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Csa = nn.Parameter(torch.tensor([RGBlimp_params['Csa']], dtype=torch.double,requires_grad=True).to(self.device))
        self.Csb = nn.Parameter(torch.tensor([RGBlimp_params['Csb']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmx0 = nn.Parameter(torch.tensor([RGBlimp_params['Cmx0']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmxa = nn.Parameter(torch.tensor([RGBlimp_params['Cmxa']], dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmxb = nn.Parameter(torch.tensor([RGBlimp_params['Cmxb']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmz0 = nn.Parameter(torch.tensor([RGBlimp_params['Cmz0']],dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmza = nn.Parameter(torch.tensor([RGBlimp_params['Cmza']], dtype=torch.double,requires_grad=True).to(self.device))
        self.Cmzb = nn.Parameter(torch.tensor([RGBlimp_params['Cmzb']],dtype=torch.double,requires_grad=True).to(self.device))
    ...
    def forward(self, z):
        ...
        ff=((self.m+self.mb)*torch.cross(v,w)).reshape((-1,3,1))+\
           ...
           Rbi@torch.tensor([[0],[0],[(self.m+self.mb-self.B)*self.g]],dtype=torch.double,requires_grad=False).to(self.device)+ \
        ...
        # tt bs*3*1
        tt=torch.cross((self.I-self.mb*skew(rb,self.device)@skew(rb,self.device))@w.unsqueeze(2),w.unsqueeze(2))+torch.cross(lg,torch.cross(v,w)).unsqueeze(2)+\
           torch.cross(lg,(Rbi@torch.tensor([[0],[0],[self.g]],dtype=torch.double,requires_grad=False).to(self.device)).squeeze(2)).unsqueeze(2)+ \
        ...
        tt=tt.squeeze(2)
        # H bs*6*6
        H_row1=torch.stack([((self.m+self.mb)*torch.eye(3).to(self.device)).repeat(bs,1,1),-skew(lg,self.device).to(self.device)],2).reshape(bs,3,6)
        H_row2=torch.stack([skew(lg,self.device).to(self.device), self.I - self.mb * skew(rb,self.device).to(self.device) @ skew(rb,self.device).to(self.device)],2).reshape(bs,3,6)
